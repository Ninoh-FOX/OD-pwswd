...
void check_battery_dead(void)
{
	...
	if((current_time-prev_time)>30000000)		// microseconds 30000000=30 seconds
	{
		...
		levelHandle = fopen("/sys/class/power_supply/jz-battery/voltage_now", "r");
		if(levelHandle)
		{
			fscanf(levelHandle, "%d", &batt_level);
			fclose(levelHandle);
		}
		chargingHandle = fopen("/sys/devices/platform/power_supply/usb-charger/online", "r");
		if(chargingHandle)
		{
		...
		if(batt_level<=340000 && batt_charging==0)
		{
			do_poweroff();
		}
	}
